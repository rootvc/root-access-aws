...
```bash
aws cloudformation delete-stack --stack-name root-access-aws
```
...
CLI output formatting is broken on OS X when the output contains as a JSON string. Fix it with the following hack:
`sam build && sam local invoke "<function>" -e events/<test-message>.json -n environment.json 2>&1 | tr "\r" "\n"`